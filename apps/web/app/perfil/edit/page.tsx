...
export default function EditProfilePage() {
  ...
  // 1. Estado para los campos del formulario (coincidiendo con tu diseño)
  ...
  // 2. Cargar datos actuales al entrar (GET)
  useEffect(() => {
    async function fetchProfile() {
      try {
        const response = await fetch('/api/perfil-empresa'); // Ruta que debería tener Cristóbal
        if (response.ok) {
  ...
  // 3. Función para Guardar Cambios (PUT o POST)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => router.push('/sesion'), 2000);
      } else {
      ...
      <main className="flex-grow container mx-auto px-10 py-12 max-w-7xl">
        <div className="grid grid-cols-12 gap-12">
          ...
          <aside className="col-span-12 md:col-span-4 lg:col-span-3 space-y-8">
            ...
            {/* Feedback Visual */}
            {mensaje.texto && (
              <div className={`p-4 rounded-lg text-[10px] font-black uppercase tracking-widest ${
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col space-y-2">
                    <label className="text-[10px] font-black text-[#434653] uppercase tracking-widest">RUT</label>
                    <input 
                      className="border border-[#737784] p-4 bg-white rounded-lg focus:border-[#0047ab] outline-none text-sm" 
                      type="text"
                      value={formData.rut}
                  ...
                  <div className="flex flex-col space-y-2 col-span-1 md:col-span-2">
                    ...
                    <select 
                      className="border border-[#737784] p-4 bg-white rounded-lg focus:border-[#0047ab] outline-none text-sm appearance-none" 
              ...
              <div className="pt-8 flex justify-end gap-6">
                <button 
                  onClick={() => router.back()}
                  className="text-[11px] font-black uppercase tracking-widest text-[#434653]" 
                  type="button"
                >
                  Cancelar
                </button>
                <button 
                  disabled={loading}
                  className="bg-[#0047ab] text-white px-10 py-4 text-[11px] font-black uppercase tracking-[0.2em] rounded-lg hover:bg-[#00327d] transition-all disabled:opacity-50 shadow-lg" 
                  type="submit"
                >
