...
interface PayloadData { 
  fecha: string; busqueda: Busqueda; 
  empresas: Empresa[]; 
...
interface Execution { 
  ...
  status: string | null; 
  summary: string; payload: PayloadData; 
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await db
      .select()
      .from(analisis)
      .where(eq(analisis.tiendaId, usuario.tiendaActivaId))
    ...
    const executions: Execution[] = result.map((exec) => {
      ...
      return { id: exec.id, fechaEjecucion: exec.fechaEjecucion, status: exec.status, summary, payload };
    });
